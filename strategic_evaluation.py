# Start of adjusting this so that it works with strategy be two separate matrix , one for each position, exactly like
...
def world_node_strategic_evaluation(strategy, game):
    ...
    world_state_values = game.terminal_values_all_node.copy()
...
test_start_one = test_start[0, :, :]*test_start[1, :, :]
KK = BettingGame(bet_size=0.5, max_number_of_bets=2,

                                deck={0: 1, 1: 1, 2: 1}, deal_from_deck_with_substitution=False)

KTS = world_node_strategic_evaluation(test_start_one, KK)
